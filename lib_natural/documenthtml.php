...
JLoader::import('joomla.utilities.utility'); //Natural: Modernised
JLoader::import('joomla.document.document'); //Natural: Added
...
class JDocumentHTML extends JDocument
{
	 ...
	 * @since  11.1
	 */
	public $template = '';	//Natural: lets be clear, this is a string.
	public $nv = null; 	//Natural: the DOM alternative.
	...
	protected function _loadTemplate($directory, $filename)
	{
		...
		//Natural: load up the view if there is one.
		//Natural: Natural -OR- treat as normal JDocumentHTML
		//Natural: normally we could use initView to do this for us..
		$xview = $directory . '/' . basename($filename,".php") . '.xhtml';
		if (file_exists($xview)) {
			$this->nv = new NView($xview);
		}
	...
	protected function _parseTemplate()
	{
		...
		{ //Natural:: special
			$tt_f = array();
			$tt_l = array();
			$mml = $this->nv->get("//*[@data-jdoc]");
			if ($mml instanceof DOMNodeList) {
				foreach($mml as $mm) {
					$this->_doJDoc($this->nv, $mm, $tt_f, $tt_l);
				}
			} else {
				if ($mml instanceof DOMNode) {
					$this->_doJDoc($this->nv, $mml, $tt_f, $tt_l);
				}
			}
			$this->_template_tags = array_merge($tt_f,$tt_l);
		}
		return $this;
	}
	...
	protected function _doJDoc( &$v, &$m, &$f, &$l ) {
		...
		if ($type == 'module' || $type == 'modules') {
			array_push($f,array('type' => $type, 'name' => $name, 'attribs' => $attribs));
		} else {
			array_push($l,array('type' => $type, 'name' => $name, 'attribs' => $attribs));
		}
	}
	...
	if ( is_null($this->nv) ) { //Natural: classic
		...
		return str_replace($replace, $with, $this->_template);
			...
			{
				$xpath = "//*[@data-jdoc='" . $args['type'] . "']";
				if (! is_null($args['name'] ) ) {
					$xpath .= "[@data-name='" . $args['name'] . "']";
				}
				$with = array($this->getBuffer($args['type'], $args['name'], $args['attribs']));
